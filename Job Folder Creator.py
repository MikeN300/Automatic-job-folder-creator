...
while loopBack == True:
	...
	def makeFolders(folderName):
		concatFoldersPath = path + '/' + clientCode + '/' + jobNumber + ' ' +  jobName + '/' + jobNumber + '_' + folderName
		os.mkdir(concatFoldersPath, mode = Oo777)
	concatJobPath = path + '/' + clientCode + '/' + jobNumber +  jobName
	os.mkdir(concatJobPath)
	makeFolders(PDF)
	makeFolders(SRC)
	makeFolders(Layout)
	makeFolders(Links)
